...
class Square: public object
{
    ...
    virtual bool intersect(const Ray* ray, vec3& intersection, vec3& normal, vec3& color)
    {
        ...
        if(normal_texture.loaded())
        {
            normal = co.inv_trans_vector(normal_texture.get_color(hit_point.x / width, hit_point.y / height)).normalize();
        }
        else normal = this->normal(vec3());
